...
type Category = 'watch' | 'movies' | 'games' | 'food' | 'places' | 'dates' | 'jokes' | 'alarm' | 'other';
...
export default function Home() {
  ...
  useRealtimeSync({
    onSync: (event) => {
      ...
      if (event.type === 'item_created') {
        ...
        if (event.data.createdBy !== userProfile) {
          NotificationService.showNotification(`Novo item adicionado por ${event.data.createdBy}! 💕`, {
  ...
  // Timer para verificar lembretes locais (alarmes)
  useEffect(() => {
    // Pede permissão logo de início
    NotificationService.requestPermission();

    const checkReminders = () => {
      const now = new Date();
      const currentTime = now.getHours().toString().padStart(2, '0') + ':' + now.getMinutes().toString().padStart(2, '0');
      const currentDayMap: Record<number, string> = { 0: 'sun', 1: 'mon', 2: 'tue', 3: 'wed', 4: 'thu', 5: 'fri', 6: 'sat' };
      const currentDay = currentDayMap[now.getDay()];

      items.forEach(item => {
        if (item.category === 'alarm' && item.reminderActive && item.reminderTime) {
          // Verifica se o lembrete é para hoje e para a hora atual
          const isToday = !item.reminderDays || item.reminderDays.length === 0 || item.reminderDays.includes(currentDay);

          if (isToday && item.reminderTime === currentTime) {
            const userProfile = localStorage.getItem('userProfile');
            const isForMe = (userProfile === 'Mateus' && item.reminderForMateus) ||
              (userProfile === 'Amanda' && item.reminderForAmanda) ||
              // Se não especificou para quem, avisa os dois
              (!item.reminderForMateus && !item.reminderForAmanda);

            if (isForMe) {
              const notifiedKey = `notified_${item.id}_${now.toDateString()}_${currentTime}`;
      ...
      >
        ...
        {loading ? (
          ...
          <div className="px-6">
            ...
            {/* Done Section - não mostrar para categoria alarm */}
            {doneItems.length > 0 && activeCategory !== 'alarm' && (
              <div className="mt-8">
