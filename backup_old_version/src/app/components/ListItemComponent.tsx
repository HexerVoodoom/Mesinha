...
export function ListItemComponent({
  ...
  const formattedDate = format(new Date(item.createdAt), 'd MMM', { locale: ptBR });
    ...
    <Card variant="white" className="overflow-visible">
      <CardContent className="p-[18px]">
        ...
        <div className="flex items-start gap-4">
          {/* Checkbox - não mostrar para categoria dates */}
          {!isDateCategory && (
            <button
              onClick={() => {
                if (isAlarmCategory) {
                ...
                <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                  {item.eventDate && (
                    <span className="text-xs text-[#8A847D]">
                      {format(new Date(item.eventDate), 'd MMM yyyy', { locale: ptBR })}
                    </span>
                  )}
